...
```{r, out.width="80%", fig.align='center'}
# we use 'identity' when we build barplot with x and y values, by defult it requires only x values
penguins %>% 
  group_by(species) %>%
  summarise(count=n()) %>%
  ggplot(aes(x=species, y=count, color=species)) +
...
```
My friends and foes we have a plot! Only... the colors are not quite alright. What happend? ggplot uses two different parameters for setting colors, in general *color*/*colour* defines the color of the geom outline and *fill* specifies what color the geom is filled with. Let's correct it!
...
Let's try adding labels with specifc count to our plot with *geom_label* (We can also use *geom_text*).
```{r, out.width="80%", fig.align='center'}
penguins %>% 
  group_by(species) %>%
  summarise(count=n()) %>%
...
\  
We definetely cannot cover all the different way you can improve your plot, but here are some examples of possible customizations:
...
- adding new *theme** to our plot (quick change of plot appearence, see more [*here*](https://ggplot2.tidyverse.org/reference/ggtheme.html) and [*here*](https://github.com/jrnold/ggthemes))
...
The plot looks nice, but unexpectly we have discovered that some penguins' sex was not identified. This is a good opportunity to practise some data filtering.
...
penguins %>%
  group_by(species) %>%
  mutate(ratio=body_mass_g/flipper_length_mm) %>%
  summarise(avg_reatio = mean(ratio, na.rm = T))
```

Our results sure look promising, but we cannot really tell only based only on means. Let's plot the data! What type of plot do we want to use? How about we show every penguin as a datapoint? 
...
This is great! It looks like there is some correlation here. Also *Gentoo* penguis are clearly the biggest birds.
...
penguins %>%
  filter(!is.na(body_mass_g) & !is.na(flipper_length_mm)) %>%
  ggplot() +
  geom_point(aes(x=body_mass_g, y=flipper_length_mm, color=species), size=3) + # increase size of points
  geom_smooth(aes(x=body_mass_g, y=flipper_length_mm, group=species, color=species), method = "lm", se=F) +
  labs(x= "Body mass [g]", y= "Flipper length [mm]", title = "Heavier penguins have bigger wingspan", color="") +
  theme_bw() +
  theme(legend.position = "bottom") +
  scale_color_manual(values = pnw_palette("Bay",3)) # here we use 'color' palette not 'fill' palette
```
...
### 5. What is the distribution of bill length accross species?
...
The answear to this questions is not always immediately obvious. The most important thing to consider is what are the types of variables we want to display. Take a look at this figure (also created by [*Coding Club*](https://ourcodingclub.github.io)).
...
This is an improvement, but if we want to also see our specific datapoint we can do it with a variation of *geom_point* called *geom_jitter* (it adds a bit of noise to points position, so we will be able to see our observations).

```{r, out.width="80%", fig.align='center'}
penguins %>%
  filter(!is.na(bill_length_mm)) %>%
  ggplot() +
  geom_boxplot(aes(x=species,y=bill_length_mm, fill=species), outlier.alpha = 0) + # hide outliers
  geom_jitter(aes(x=species, y=bill_length_mm), width = 0.2, height = 0)  # we restrict variation of point height
```
...
penguins %>%
  ...
  geom_jitter(aes(x=species, y=bill_length_mm), width = 0.2, height = 0, size=2, alpha=0.4) + # add transparency to points
...
### 6. Importace of reshaping your data

The *palmerpenguins* dataset is already preprocessed nd prepared for analysis. This is not always the case. When using your own data frequently check if your observations are in correct orientation (rows or columns). This is extremely importatant when plotting data, because most of the operations is based on assumption of variables being in columns of the tibble.
